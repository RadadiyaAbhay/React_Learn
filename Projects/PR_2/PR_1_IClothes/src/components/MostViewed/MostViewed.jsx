...
export default function MostViewed() {
  ...
  return (
    <div className="bg-white">
      <div className="mx-auto max-w-2xl px-4 pt-16 sm:px-6 sm:pt-16 lg:max-w-7xl lg:px-8">
        <div className="flex items-end justify-between">
          <h2 className="text-2xl font-bold tracking-tight text-gray-900">Popular Now</h2>
          <a className="font-semibold text-md flex items-center point" onClick={()=>{handleRedirect('New Arrival')}}>View More <BiSolidChevronsRight className="ms-1 text-xl font-gray-100" /></a>
        </div>
        <div className="mt-4 mb-16 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          {products.map((product, index) => (
            index < 4 ? (<div key={product.id} className="point group relative addto">
          ...
          </div>) : (<span className="hidden" key={product.id}></span>)
            )
          )}
        </div>
      </div>
