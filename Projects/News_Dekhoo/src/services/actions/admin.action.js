...
import { ADMINLOGINREJ, ADMINLOGINREQ, ADMINLOGINRES, ADMINSIGNOUTREJ, ADMINSIGNOUTREQ, ADMINSIGNOUTRES, SINGLEADMINREJ, SINGLEADMINREQ, SINGLEADMINRES } from "../const"
...
export const adminSignOutReq = () => {
    return {
        type: ADMINSIGNOUTREQ
    }
}
export const adminSignOutRej = (err) => {
    return {
        type: ADMINSIGNOUTREJ,
        payload: err
    }
}
export const adminSignOutRes = () => {
    return {
        type: ADMINSIGNOUTRES
    }
}
export const adminSignOutAction = () => {
    return async dispatch => {
        dispatch(adminSignOutReq())
        await signOut(auth).then(() => {
            dispatch(adminSignOutRes())
        }).catch((err) => {
            dispatch(adminSignOutRej(err))
        })
    }
}
